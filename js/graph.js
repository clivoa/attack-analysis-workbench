...
const Graph = (() => {
  ...
  function rebuild() {
    ...
    sim.nodes(nodes);
    sim.force("link", d3.forceLink(links).id((d) => d.id).distance(70).strength(0.4));
  ...
  function init() {
    ...
    sim = d3.forceSimulation()
      ...
      .force("collide", d3.forceCollide((d) => NODE_R[typeOf(d.id)] + 8))
    ...
    document.getElementById("graph-example").addEventListener("click", () => {
      ...
      setTimeout(() => { expand("G0016"); Panel.show("G0016"); }, 100);
    });
