...
function profileHas(id) { return S.profile.has(id); }

function profileToggle(id) {
...
function profileChanged() {
  ...
  if (S.view === "matrix") Matrix.render();
  if (S.view === "defense") Defense.render();
  if (S.view === "detect") Detect.render();
  if (S.selected) Panel.refreshActions();
}
...
function renderProfileChips() {
  ...
  wrap.querySelectorAll(".profile-chip").forEach((c) =>
    c.addEventListener("click", () => Panel.show(c.dataset.id)));
}
...
function switchView(view) {
  ...
  if (view === "matrix" && S.dirty.matrix) Matrix.render();
  if (view === "defense" && S.dirty.defense) Defense.render();
  if (view === "detect" && S.dirty.detect) Detect.render();
  if (view === "explore") Graph.onShow();
}
...
function setupSearch() {
  ...
  function pick(id) {
    ...
    input.blur();
    Panel.show(id);
    if (S.view === "explore") Graph.addEntity(id, { focus: true });
  }
...
async function init() {
  ...
  document.getElementById("profile-graph").addEventListener("click", () => {
    switchView("explore");
    [...S.profile].forEach((id) => Graph.addEntity(id));
  });
  ...
  if (["explore", "matrix", "detect", "defense"].includes(hash)) {
    ...
    Tour.start(key, parseInt(at) || 0);
  } else if (hash.startsWith("detect=") && getEnt(hash.slice(7))) {
    Detect.focus(hash.slice(7)); // e.g. #detect=T1059.001 → detection card
  } else if (hash && getEnt(hash)) {
    Panel.show(hash);
    if (typeOf(hash) !== "tactic") Graph.addEntity(hash, { focus: true });
  }
}
