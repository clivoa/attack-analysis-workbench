...
import { S, esc, switchView, profileChanged } from './app.js'
...
export const Tour = (() => {
    ...
    overview: {
      ...
      steps: [
        ...
        },
        {
          ...
          before: async () => window.Panel.show("G0016"),
    ...
    cti: {
      ...
      steps: [
        ...
        },
        {
          ...
          before: async () => {
            window.Graph.clear();
            window.Graph.addEntity("G0016");
            window.Graph.addEntity("G0032");
            await sleep(120);
            window.Graph.expand("G0016");
            await sleep(400);
          },
    ...
    engineer: {
      ...
      steps: [
        ...
        },
        {
          ...
          before: async () => { window.Detect.applyPreset("Windows"); await sleep(100); },
        ...
        },
        {
          ...
          before: async () => { window.Detect.focus("T1053.005"); await sleep(150); },
        ...
        },
        {
          ...
          before: async () => { window.Detect.clearFilter(); await sleep(100); },
  ...
  function snapshot() {
    return { profile: [...S.profile], telemetry: window.Detect.getTelemetry() };
  }

  function restore(snap) {
    S.profile = new Set(snap.profile);
    profileChanged();
    window.Detect.setTelemetry(snap.telemetry);
    window.Detect.clearFilter();
    window.Graph.clear();
  }
